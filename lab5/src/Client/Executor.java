...
public class Executor {
    ...
    public void executeStringCmd(String command_str) {
        ...
        if (commandsMap.containsKey(cmd_with_args[0]))
            ...
            printSender.send("Ошибка: Неизвестная команда. Введите help для справки.");
    ...
    public void Run() {
        while (true) {
            executeStringCmd(this.inputRequester.request("Команда: "));
        }
    }
    ...
    public CollectionManager<Vehicle> getCollection() {
        return collection;
    }
    ...
    public StringRequesterInterface getInputRequester() {
        return inputRequester;
    }

    public StringRequesterInterface getFileRequester() {
        return fileRequester;
    }

    public StringSenderInterface getPrintSender() {
        return printSender;
    }

    boolean checkAndFixCommandArgs(int argc) {
        if (this.cmd_with_args.length < argc + 1) {
            printSender.send("Error: Not enough arguments");
            return false;
        }

        if (this.cmd_with_args.length > argc + 1)
            printSender.send("Warning: Too many arguments");
    ...
    void info() {
        printSender.send(collection.toPrettyString());
    }
